...
while run:
    if state == 'game':
        ...
        pygame.draw.polygon(screen, (188, 204, 198),
                            [(w/2 - tx - size / 2, h/2 - ty - size / 2), (w/2 - tx - size / 2, h/2 - ty + size / 2),
                             (w/2 - tx + size / 2, h/2 - ty + size / 2), (w/2 - tx + size / 2, h/2 - ty - size / 2)])
        for val in range(size // 25 + 1):
            pygame.draw.line(screen, (204, 216, 211), (w/2 - tx - size / 2, h/2 - ty + 25 * val - size / 2),
                             (w/2 - tx + size / 2, h/2 - ty + 25 * val - size / 2), 2)
            pygame.draw.line(screen, (204, 216, 211), (w/2 - tx + 25 * val - size / 2, h/2 - ty - size / 2),
                             (w/2 - tx + 25 * val - size / 2, h/2 - ty + size / 2), 2)
        ...
        newdarts = []
        for d in darts:
            if d[1][0] >= 30 and (not (use_power[0] and power == 'homing') or math.dist(((mx + tx - w/2), (my + ty - h/2)), (d[0][0], d[0][1])) > 3):
                newdarts.append(d)
        darts = newdarts[:]
        ...
        if not use_power[0]:
            ...
            screen.fill((247, 120, 22), pygame.Rect(w/2-100, h-50, 200 * (1 - use_power[1] / powers[power][1]), 15))
